...
defineFeature(feature, (test) => {
  ...
  test('An event element is collapsed by default', ({ given, when, then }) => {
    given(
      'the user has opened the app and chosen a city to find events',
      async () => {
        render(<App />)
    ...
    then(
      'each event element should be collapsed by default and the event details should not be visible',
      () => {
  ...
  test('The User can collapse an event to hide details', ({
    ...
    given('the user has expanded an event to view its details', async () => {
      render(<App />)
      await waitFor(() =>
        expect(screen.getAllByTestId('event')).not.toHaveLength(0)
      )
    ...
    then(
      "the details of that event should be hidden and the event should display it's collapsed default state.",
      () => {
